...
namespace ClientCore
{
    public partial class ContextSystem
    {
        ...
        {
            var prtGachaSchedule = ProtoDb.Get<GachaScheduleProto>(scheduleNum);
        ...
        {
            var prtCookie = ProtoDb.Get<CookieProto>(cookieNum);
            var cookie = GetCookieForce(cookieNum);
            var useSoulStone = 0;

            for (var star = cookie.Star; star < aftStar; star++)
            {
                var prtCookieStarEnhance = ProtoDb.Get<CookieStarEnhanceProto>((prtCookie.GradeType, star));
